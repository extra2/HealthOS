// <auto-generated />
using HealthOS.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage;
using Microsoft.EntityFrameworkCore.Storage.Internal;
using System;

namespace HealthOS.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20181105135115_addedCustomStatistics")]
    partial class addedCustomStatistics
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.0.3-rtm-10026")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("HealthOS.Models.Allergy", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<string>("Value");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Allergies");
                });

            modelBuilder.Entity("HealthOS.Models.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("AccessFailedCount");

                    b.Property<DateTime>("Birthday");

                    b.Property<string>("BloodType");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Email")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed");

                    b.Property<string>("Gender");

                    b.Property<string>("InvitationCode");

                    b.Property<bool>("LockoutEnabled");

                    b.Property<DateTimeOffset?>("LockoutEnd");

                    b.Property<string>("Name");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash");

                    b.Property<string>("PhoneNumber");

                    b.Property<bool>("PhoneNumberConfirmed");

                    b.Property<string>("SecurityStamp");

                    b.Property<string>("Surname");

                    b.Property<bool>("TwoFactorEnabled");

                    b.Property<string>("UserName")
                        .HasMaxLength(256);

                    b.Property<bool>("isDoctor");

                    b.Property<bool>("isPatient");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("HealthOS.Models.ChatMessage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Body");

                    b.Property<bool>("Seen");

                    b.Property<DateTime>("SentDate");

                    b.Property<string>("Title");

                    b.Property<string>("UserFromId");

                    b.Property<string>("UserToId");

                    b.HasKey("Id");

                    b.HasIndex("UserFromId");

                    b.HasIndex("UserToId");

                    b.ToTable("ChatMessages");
                });

            modelBuilder.Entity("HealthOS.Models.Disease", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<string>("Description");

                    b.Property<DateTime?>("EndDate");

                    b.Property<string>("Name");

                    b.Property<DateTime>("StartDate");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Diseases");
                });

            modelBuilder.Entity("HealthOS.Models.Drug", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Amount");

                    b.Property<string>("ApplicationUserId");

                    b.Property<string>("ConsumptionMethod");

                    b.Property<bool>("Day1");

                    b.Property<bool>("Day2");

                    b.Property<bool>("Day3");

                    b.Property<bool>("Day4");

                    b.Property<bool>("Day5");

                    b.Property<bool>("Day6");

                    b.Property<bool>("Day7");

                    b.Property<string>("Name");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Drugs");
                });

            modelBuilder.Entity("HealthOS.Models.NextDoctorVisit", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<string>("Description");

                    b.Property<string>("DoctorName");

                    b.Property<string>("Location");

                    b.Property<DateTime>("VisitTime");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("NextDoctorVisits");
                });

            modelBuilder.Entity("HealthOS.Models.Relation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AuthorizedDoctorsId");

                    b.Property<string>("AuthorizedPatientsId");

                    b.HasKey("Id");

                    b.HasIndex("AuthorizedDoctorsId");

                    b.HasIndex("AuthorizedPatientsId");

                    b.ToTable("Relations");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.BloodPressureStatistics", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<int>("Diastolic");

                    b.Property<DateTime>("MeasurementDate");

                    b.Property<int>("Systolic");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("BloodPreasures");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.CustomStatistics", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<string>("StatisticsName");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("CustomStatistics");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.CustomStatisticsData", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<int?>("CustomStatisticsId");

                    b.Property<double>("FirstMeasurement");

                    b.Property<bool>("IsSecondMeasurementUsed");

                    b.Property<DateTime>("MeasurementDate");

                    b.Property<double>("SecondMeasurement");

                    b.HasKey("Id");

                    b.HasIndex("CustomStatisticsId");

                    b.ToTable("CustomStatisticsDatas");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.GlucoseLevelStatistics", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<int>("Glucose");

                    b.Property<DateTime>("MeasurementDate");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("GlucoseLevels");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.HeightStatistics", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<int>("Height");

                    b.Property<DateTime>("MeasurementDate");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Heights");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.WeightStatistics", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserId");

                    b.Property<DateTime>("MeasurementDate");

                    b.Property<int>("Weight");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Weights");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Name")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("RoleId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider");

                    b.Property<string>("ProviderKey");

                    b.Property<string>("ProviderDisplayName");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("RoleId");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("LoginProvider");

                    b.Property<string>("Name");

                    b.Property<string>("Value");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("HealthOS.Models.Allergy", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("Allergies")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.ChatMessage", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "UserFrom")
                        .WithMany()
                        .HasForeignKey("UserFromId");

                    b.HasOne("HealthOS.Models.ApplicationUser", "UserTo")
                        .WithMany()
                        .HasForeignKey("UserToId");
                });

            modelBuilder.Entity("HealthOS.Models.Disease", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("Diseases")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Drug", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("Drugs")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.NextDoctorVisit", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("NextDoctorVisits")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Relation", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "AuthorizedDoctors")
                        .WithMany()
                        .HasForeignKey("AuthorizedDoctorsId");

                    b.HasOne("HealthOS.Models.ApplicationUser", "AuthorizedPatients")
                        .WithMany()
                        .HasForeignKey("AuthorizedPatientsId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.BloodPressureStatistics", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("BloodPressureStatistics")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.CustomStatistics", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("CustomStatistics")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.CustomStatisticsData", b =>
                {
                    b.HasOne("HealthOS.Models.Statistics.CustomStatistics", "CustomStatistics")
                        .WithMany("CustomStatisticsDatas")
                        .HasForeignKey("CustomStatisticsId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.GlucoseLevelStatistics", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("GlucoseStatistics")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.HeightStatistics", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("HeightStatistics")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("HealthOS.Models.Statistics.WeightStatistics", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("WeightStatistics")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("HealthOS.Models.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("HealthOS.Models.ApplicationUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
